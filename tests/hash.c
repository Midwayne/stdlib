...
START_TEST(test_hash_bytes_basic) {
    ...
    ck_assert_uint_eq(hash_bytes(s, strlen(s)), (hash_t)ref_fnv1a_64(s, strlen(s)));
#else
    ck_assert_uint_eq(hash_bytes(s, strlen(s)), (hash_t)ref_fnv1a_32(s, strlen(s)));
#endif
}
END_TEST
...
#if SIZE_MAX == UINT64_MAX
    ck_assert_uint_eq(hash_bytes(data, sizeof(data)), (hash_t)ref_fnv1a_64(data, sizeof(data)));
#else
    ck_assert_uint_eq(hash_bytes(data, sizeof(data)), (hash_t)ref_fnv1a_32(data, sizeof(data)));
#endif
}
END_TEST
...
#if SIZE_MAX == UINT64_MAX
    ck_assert_uint_eq(hash_bytes(&dummy, 0), (hash_t)ref_fnv1a_64(&dummy, 0));
#else
    ck_assert_uint_eq(hash_bytes(&dummy, 0), (hash_t)ref_fnv1a_32(&dummy, 0));
#endif
}
END_TEST
...
START_TEST(test_hash_string) {
    ...
    ck_assert_uint_eq(hash_string(s), hash_bytes(s, strlen(s)));
}
END_TEST
...
START_TEST(test_hash_int) {
    ...
    ck_assert_uint_eq(hash_int(&x), hash_bytes(&x, sizeof(x)));
}
END_TEST
...
START_TEST(test_hash_long) {
    ...
    ck_assert_uint_eq(hash_long(&x), hash_bytes(&x, sizeof(x)));
}
END_TEST
...
START_TEST(test_hash_ptr) {
    ...
    ck_assert_uint_eq(hash_ptr(&x), hash_bytes(&p, sizeof(p)));
}
END_TEST
